          ...
          Check_key(e)
            ...
            {message.length > 0 ? (
                message.map((data, index) => (
                  <div key={index} className='flex justify-center items-center'><img src={data.avatar} alt={data.User} className='h-5 w-5 rounded-xl' /> {data.User.split(' ')[0]} : {data?.message || ''}</div>
                ))
                  ) : (
            <p>no messages</p>
          )}
